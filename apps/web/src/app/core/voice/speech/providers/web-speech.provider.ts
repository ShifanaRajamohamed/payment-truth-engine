...
/**
 * Locale fallbacks when the OS has no native TTS voice for a scheduled language.
 * Recognition still uses the requested locale; speech uses the first available voice.
 */
...
export class WebSpeechProvider implements SpeechProvider {
  ...
  startListening(locale: string): Observable<string> {
    ...
    // Stay open until the user taps stop, so a full question is captured.
  ...
  private startKeepAlive(): void {
    this.stopKeepAlive();
    // Chrome silently kills utterances around 15s unless speechSynthesis is nudged.
  ...
  private chunkForSpeech(text: string): string[] {
    ...
    // Hard-split any leftover long chunk so Chrome cannot stall mid-utterance.
    const max = 180;
    const sized: string[] = [];
    for (const chunk of chunks) {
      if (chunk.length <= max) {
