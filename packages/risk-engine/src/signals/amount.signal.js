...
exports.detectAmountSignals = detectAmountSignals;
function detectAmountSignals(ctx) {
    const signals = [];
    ...
    if (ctx.historicalPayments && ctx.historicalPayments.length > 0) {
        const beneficiaryTxs = ctx.historicalPayments.filter(p => p.beneficiaryId === ctx.payment.beneficiaryId && p.status === 'SUCCESS');
